...
const BACKEND_URL = process.env.BACKEND_URL || 'http://edm-backend-prod:3000';

// 数据库配置
const sequelize = new Sequelize({
  dialect: 'postgres',
  host: process.env.DB_HOST || 'edm-postgres-prod',
...
const redisClient = redis.createClient({
  url: `redis://${process.env.REDIS_HOST || 'edm-redis-prod'}:${process.env.REDIS_PORT || 6379}`
});
...
/**
 * SubTask邮件打开追踪 - V2.0 (EDM系统)
 * GET /track/open/:subTaskId
 */
app.get('/track/open/:subTaskId', async (req, res) => {
  try {
    const { subTaskId } = req.params;

    console.log(`📧 收到SubTask打开追踪请求: ${subTaskId}`);

    // 调用后端API更新SubTask状态
    try {
      const response = await axios.get(`${BACKEND_URL}/api/tracking/open/${subTaskId}`, {
        headers: {
          'User-Agent': req.headers['user-agent'] || 'tracking-service',
          'X-Real-IP': getClientIP(req),
          'X-Forwarded-For': req.headers['x-forwarded-for'] || ''
        }
      });
      console.log(`✅ SubTask ${subTaskId} 打开追踪成功`);
    } catch (apiError) {
      console.error(`❌ SubTask ${subTaskId} 后端API调用失败:`, apiError.message);
    }
    ...
    console.error('SubTask追踪失败:', error);
...
/**
 * SubTask邮件打开追踪 - API路径支持 (兼容nginx /api/tracking/ 路由)
 * GET /api/tracking/open/:subTaskId
 */
app.get('/api/tracking/open/:subTaskId', async (req, res) => {
  // 重定向到标准路径
  return app._router.handle(
    { ...req, url: `/track/open/${req.params.subTaskId}`, path: `/track/open/${req.params.subTaskId}` },
    res
  );
});
