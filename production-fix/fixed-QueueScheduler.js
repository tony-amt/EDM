...
const { Op } = Sequelize;
const EmailRoutingService = require('./EmailRoutingService.service');
const QuotaService = require('./QuotaService.service');
    ...
    switch (type) {
      ...
      case 'tag_based':
        ...
        if (include_tags && include_tags.length > 0) {
          // 修复：强制类型转换JSONB数组
          whereClause.tags = { [Op.overlap]: Sequelize.cast(JSON.stringify(include_tags), 'jsonb') };
        }
        if (exclude_tags && exclude_tags.length > 0) {
          if (whereClause.tags) {
            whereClause[Op.and] = [
              { tags: whereClause.tags },
              { [Op.not]: { tags: { [Op.overlap]: Sequelize.cast(JSON.stringify(exclude_tags), 'jsonb') } } }
            ];
            delete whereClause.tags;
          } else {
            whereClause[Op.not] = { tags: { [Op.overlap]: Sequelize.cast(JSON.stringify(exclude_tags), 'jsonb') } };
          }
        }
        return await Contact.findAll({
          where: whereClause,
