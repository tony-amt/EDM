...
class QueueScheduler {
  ...
  async getTaskContacts(task, transaction) {
    ...
    switch (type) {
      ...
      case 'tag_based':
        let whereClause = { user_id: task.created_by };

        if (include_tags && include_tags.length > 0) {
          // 修复：使用?|操作符检查JSONB数组是否包含任一指定标签
          const tagIds = include_tags.map(id => `'${id}'`).join(',');
        ...
        if (exclude_tags && exclude_tags.length > 0) {
          // 修复：使用NOT和?|操作符排除包含指定标签的联系人
          const excludeIds = exclude_tags.map(id => `'${id}'`).join(',');
          whereClause[Op.and] = whereClause[Op.and] || [];
          whereClause[Op.and].push(
  ...
  async createSubTaskQueue(task, contacts, transaction) {
    ...
    for (const contact of contacts) {
      ...
      subTasksData.push({
        ...
        scheduled_at: null,
        allocated_quota: 1,
        tracking_id: uuidv4(), // 修复：添加tracking_id字段
        tracking_data: {}
      });
    }
