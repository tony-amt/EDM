...
const { Task, SubTask, EmailService, User, Sender } = require('./src/models');
...
class SchedulerHealthChecker {
   ...
   * 检查调度器运行状态
   */
  async checkSchedulerRunning() {
    try {
      // 尝试检查调度器API
      const QueueScheduler = require('./src/services/infrastructure/QueueScheduler');
      const scheduler = new QueueScheduler();
      const status = scheduler.getStatus();
      
      this.addCheck('调度器运行状态', 'OK', 
        `调度器运行状态: ${status.is_running ? '运行中' : '已停止'}`, 
        status);

      if (!status.is_running) {
        this.addCheck('调度器服务', 'ERROR', '调度器未运行');
        this.addRecommendation('紧急：需要启动调度器服务');
      } else {
        if (status.active_services === 0) {
          this.addCheck('活跃发信服务', 'WARNING', '没有活跃的发信服务定时器');
          this.addRecommendation('检查发信服务配置和启动状态');
        }
      }

      return true;
    } catch (error) {
      this.addCheck('调度器运行检查', 'WARNING', '无法检查调度器运行状态', { error: error.message });
      return false;
    }
  }
  ...
  async runHealthCheck() {
    ...
    await this.checkSchedulerRunning();
