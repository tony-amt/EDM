...
exports.getContacts = async (req, res, next) => {
  try {
    // query parameters are directly passed as filters object to the service
    const result = await ContactService.getContacts(req.query, req.user.id);
...
exports.getContact = async (req, res, next) => {
  try {
    const contactDto = await ContactService.getContactById(req.params.id, req.user.id);
...
exports.exportContacts = async (req, res) => {
  try {
    ...
    const exportData = contacts.map(contact => ({
      ...
      tags: (contact.tags || []).map(tagId => tagMap[tagId] || tagId).join(', '),
