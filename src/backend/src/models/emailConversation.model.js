...
const EmailConversation = sequelize.define('EmailConversation', {
  ...
  // 定义关联关系
  EmailConversation.associate = function(models) {
    // 会话属于用户
    EmailConversation.belongsTo(models.User, {
      foreignKey: 'user_id',
      as: 'user'
    });

    // 会话有多个消息
    EmailConversation.hasMany(models.EmailMessage, {
      foreignKey: 'conversation_id',
      as: 'messages'
    });

    // 会话属于发信服务
    EmailConversation.belongsTo(models.EmailService, {
      foreignKey: 'email_service_id',
      as: 'emailService'
    });
  };
