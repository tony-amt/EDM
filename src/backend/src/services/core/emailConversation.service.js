const { EmailConversation, EmailMessage, User, Sender, EmailService } = require('../../models/index');
...
class EmailConversationService {
  ...
  async handleInboundEmail(emailData) {
    ...
    try {
      const {
        ...
        custom_args,
        received_at,
        sender_email // 新增：原始发送邮件的sender@domain
      } = emailData;

      // 1. 根据sender_email或to_email匹配用户账号
      let user = null;
      let actualSenderEmail = to_email; // 默认使用to_email

      // 优先通过sender_email识别用户（适用于邮件回复场景）
      if (sender_email) {
        user = await this.findUserBySenderEmail(sender_email);
        actualSenderEmail = sender_email;
        logger.info(`通过sender_email识别用户: ${sender_email} -> ${user ? user.email : '未找到'}`);
      }

      // 如果通过sender_email没找到，回退到原有逻辑
      if (!user) {
        user = await this.findUserByEmail(to_email);
        actualSenderEmail = to_email;
        logger.info(`通过to_email识别用户: ${to_email} -> ${user ? user.email : '未找到'}`);
      }

      if (!user) {
        logger.warn(`未找到匹配的用户账号: sender=${sender_email}, to=${to_email}`);
        return null;
      }
      ...
      const conversation = await this.findOrCreateConversation({
        ...
        original_sender_email: sender_email // 保存原始sender信息
      }, transaction);
  ...
  /**
   * 根据sender邮箱地址查找用户（用于邮件回复场景）
   */
  async findUserBySenderEmail(senderEmail) {
    if (!senderEmail) return null;

    // 解析sender@domain格式
    const [senderName, domain] = senderEmail.split('@');

    if (!senderName || !domain) return null;

    logger.info(`解析sender邮箱: ${senderEmail} -> sender=${senderName}, domain=${domain}`);

    // 1. 通过senders.name查找对应的用户
    ...
    if (sender && sender.user) {
      logger.info(`通过Sender.name找到用户: ${sender.user.email} (sender: ${senderName})`);
      return sender.user;
    }

    // 2. 如果没有找到sender记录，尝试通过EmailService的domain验证是否为有效域名
    ...
    if (!emailService) {
      logger.warn(`未找到域名对应的邮件服务: ${domain}`);
      return null;
    }

    // 3. 如果域名有效但没有对应的sender，返回第一个管理员用户
    // 这种情况可能是系统配置问题或测试数据
    const adminUser = await User.findOne({
      where: { role: 'admin' }
    });
    ...
    if (adminUser) {
      logger.warn(`Sender ${senderName} 不存在，使用默认管理员用户: ${adminUser.email}`);
      return adminUser;
    }

    logger.error(`无法识别用户: sender=${senderName}, domain=${domain}`);
    return null;
  }
